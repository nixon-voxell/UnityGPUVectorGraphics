        ...
        private struct ConstrainJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                for (int s = 0; s < segmentCount; s++)
                {
                    ...
                    for (int t = 0, triangleCount = na_triangles.Length / 3; t < triangleCount; t++)
                    {
                        ...
                        if (TriEdgeIntersect(in na_points, in tIdx, in edge, in ePoints, out diff_t, out tPoints))
                        {
                            ...
                            float2 n = math.normalize(mathx.perpendicular(ePoints[1] - ePoints[0]));
                            for (int i = 0; i < 3; i++)
                            {
                                if (diff_t[i])
                                {
                    ...
                    if (na_insideIndices.Length > 2)
                    {
                        TriangulatePoints(
                          ...
                          ref na_repairEdges, ref na_repairTriangles, ref na_repairCircumcenters,
                          ref na_blackListedRepairEdges, ref na_blackListedTris
                        );
                    }
                ...
                for (int s = 0; s < segmentCount; s++)
                {
                    ...
                    NativeParallelMultiHashMap<int, int>.Enumerator enumerator = na_pointTriMap.GetValuesForKey(e0);
                    ...
                    if (tris[1] != -1)
                    {
                        float2 n = math.normalize(mathx.perpendicular(na_points[edge.e1] - na_points[edge.e0]));
...

                {
                    ...
                    for (int t = 0, triangleCount = na_triangles.Length / 3; t < triangleCount; t++)
                    {
                        ...
                        NativeParallelMultiHashMap<int, int>.Enumerator enum0 = na_pointTriMap.GetValuesForKey(t0);
                        NativeParallelMultiHashMap<int, int>.Enumerator enum1 = na_pointTriMap.GetValuesForKey(t1);
                        NativeParallelMultiHashMap<int, int>.Enumerator enum2 = na_pointTriMap.GetValuesForKey(t2);
            ...
            )
            {
                ...
                for (int i = 0; i < idxCount; i++)
                {
                    ...
                    for (int c = 0, circumCount = na_repairCircumcenters.Length; c < circumCount; c++)
                    {
                        ...
                        if (circumcenter.ContainsPoint(point))
                        {
                            ...
                            AddEdgesOfRemovedTriangle(in edge, ref na_repairEdges, ref na_blackListedRepairEdges);
                    ...
                    na_blackListedRepairEdges.Sort();
                    RemoveBlacklistedEdges(in na_blackListedRepairEdges, ref na_repairEdges);

                    // create new triangles out of it by connecting
                    // each new edges to the current point
                    CreateTrianglesForNewPoint(
                      in pIdx, in point, in na_repairEdges, in na_points,
                ...
                for (int rt = 0; rt < repairTriCount; rt++)
                {
                    ...
                    // TODO: implement one third...
                    float2 triMidPoint = (na_points[t0] + na_points[t1] + na_points[t2]) * (1.0f / 3.0f);
                ...
                na_triangles.AddRange(na_repairTriangles.AsArray());
            }
        }
    }
}
